...
-- Query 4B: Retrieve a list of employees ordered in descending order by department ID and within each department ordered akphabetically in descending order by name
...
SELECT DEP_ID, COUNT(*) as count, AVG(SALARY) as avg_salary FROM EMPLOYEES GROUP BY DEP_ID;
...
-- Query 6(bonus): Similar to 4B but instead of department ID use department name. Retrieve a list of employees ordered by department name, and within each department
